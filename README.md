...
🔥 **Featured Case Study:**  
> How we refactored vLLM’s API server and cut structural boilerplate by ~50%  
> 👉 [Read: Refactoring vLLM API Architecture](https://github.com/HeHongyeFY/fastapi-cbx/wiki/refactoring-vllm-api-architecture)
---
...
- HTTP and WebSocket endpoints bound by the same decorator style
- Ultra-lightweight: ~130 lines of clean code
- 100% test coverage
...
import time
from typing import Dict
from fastapi import FastAPI, APIRouter, Response, status, Cookie, Depends, HTTPException, WebSocket
from cbx import cbr
from pydantic import BaseModel
...
class MyCBR:
    ...
    @cbr.websocket("/heavies", name="websocket heavies")  # WebSocket support
    async def websocket_heavies(self, websocket: WebSocket) -> None:
        await websocket.accept()
        key = await websocket.receive_text()
        await websocket.send_text(self.heavies.get(key, "One scenario, one route"))
