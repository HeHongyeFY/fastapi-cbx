...
class MyCBR:
    ...
    @cbr.websocket("/heavies", name="websocket heavies")
...
def test_cbr():
    ...
    with client.websocket_connect("/cbr/heavies") as websocket:
        websocket.send_text("name")
        assert websocket.receive_text() == "fastapi-cbx"
    with client.websocket_connect("/cbr/heavies") as websocket:
        websocket.send_text("unknown")
        assert websocket.receive_text() == "One scenario, one route"
